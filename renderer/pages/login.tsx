...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = (await ipcLogin({
        username: formData.username,
        password: formData.password,
      })) as { success: boolean; user: any; userRole: any; error?: string };

      if (response.success) {
        console.log(response);
        const user = response.user; // Access the user property
        const userRole = response.userRole.role_name; // Access the user property
        authLogin(user, userRole); // Pass username to auth context
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-4">
                    <div>
                      <label
                        htmlFor="username"
                        className="block text-sm font-medium text-blue-900/80"
                      >
                        Username
                      </label>
                      <input
